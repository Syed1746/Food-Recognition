...
# Route to render testimonial.html
@app.route('/testimonial')
def testimonial():
    return render_template('testimonial.html')

# Route to render service.html
@app.route('/service')
def service():
    return render_template('home.html')

# Route to render service2.html and fetch data from Django views
@app.route('/service2')
def service2():
    try:
        # Make a request to your Django views
        response = requests.get('http://127.0.0.1:8000/service2')  # Replace with your Django server URL
        # Check if the request was successful
        if response.status_code == 200:
            # Render service2.html with the response data from Django
            return render_template("service2.html", api=response.json())
        else:
            # Handle the error if the request was not successful
            return f"Error: {response.status_code}"
    except Exception as e:
        return f"Error: {str(e)}"
# Route to render blog.html
@app.route('/blog')
def blog():
    return render_template('blog.html')

# Route to render contact.html
@app.route('/contact')
def contact():
...
# Route to execute manage.py when the button is clicked
@app.route("/execute_manage_py", methods=["POST"])
def execute_manage_py():
    try:
        # Execute the manage.py script using subprocess with the modified port
        subprocess.run(["python", "manage.py", "runserver", "5000"])
        return redirect(url_for("service2"))  # Redirect back to service2.html after execution
    except Exception as e:
        return f"Error executing manage.py: {str(e)}"
